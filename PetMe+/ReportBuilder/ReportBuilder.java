...
public class ReportBuilder {
    ...
    private void addFirstPage(Pet pet) throws IOException {
        ...
        try (PDPageContentStream stream = new PDPageContentStream(document, page)) {
            ...
            stream.beginText();
            stream.newLineAtOffset(45f, page.getMediaBox().getUpperRightY()-170f);
            stream.setFont(PDType1Font.TIMES_BOLD, 32);
            stream.showText(pet.name);
            stream.endText();

            stream.beginText();
            stream.newLineAtOffset(45f, page.getMediaBox().getUpperRightY()-200f);
            stream.setFont(PDType1Font.TIMES_BOLD, 28);
            stream.showText(pet.gender);
            stream.endText();

            int age = Year.now().getValue() - pet.petBirthday.getYear();
            stream.beginText();
            stream.newLineAtOffset(45f, page.getMediaBox().getUpperRightY()-230f);
            stream.setFont(PDType1Font.TIMES_BOLD, 28);
            stream.showText(age + " years old");
            stream.endText();

            stream.beginText();
            stream.newLineAtOffset(45f, page.getMediaBox().getUpperRightY()-260f);
            stream.setFont(PDType1Font.TIMES_BOLD, 28);
            stream.showText(pet.type);
            stream.endText();
